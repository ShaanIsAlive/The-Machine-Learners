...
# ── Global styling (theme colors live in .streamlit/config.toml) ──────────
st.markdown("""
<style>
...
map_df["estimated_exposure"] = (map_df["vulnerability_score"] * 2000).round().astype(int)
...
with main_tab:
    with st.container(border=True):
        ...
        q75_mean = float(
            map_df[map_df["risk_tier"].isin(["Extreme", "High"])]["vulnerability_score"].mean() or 0
        )
        multiplier = q75_mean / max(latest_avg, 1e-6)
        c1.metric(
            "High-risk multiplier", f"{multiplier:.1f}x",
    ...
    with st.container(border=True):
        st.subheader(f"🗺️ Vulnerability Map ({latest_ym}) — {city_label}")
    ...
    with st.container(border=True):
        ...
        hi_extreme_residents = int(
            map_df.loc[map_df["risk_tier"].isin(["Extreme", "High"]), "estimated_exposure"].sum()
        )
    ...
    with st.container(border=True):
        st.subheader("🛠️ Preventive Action Engine")
...
with details_tab:
    ...
    with st.container(border=True):
        st.subheader("⚠️ When to trust and when to be careful")
        st.markdown(
